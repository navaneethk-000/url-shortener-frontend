...
<template>
  <div
    class="relative bg-slate-900/80 backdrop-blur-xl border border-white/10 p-8 sm:p-10 rounded-2xl shadow-2xl shadow-black/80"
  >
    <!-- Header -->
    <div class="mb-10 text-center">
      <div
        class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-indigo-500/10 text-indigo-400 mb-4 border border-indigo-500/20 shadow-lg shadow-indigo-500/10"
      >
        <svg
          class="w-6 h-6"
          fill="none"
    ...
    <form v-if="!isSuccess" @submit.prevent="handleLogin" class="space-y-6">
      ...
      <div class="group">
        ...
        >
        <div
          class="relative transition-all duration-300 focus-within:scale-[1.01]"
        >
          <div
      ...
      >
        <div
          v-if="errorMsg"
          class="rounded-lg bg-red-500/10 border border-red-500/20 p-4 flex items-start"
        >
          <svg
      ...
      >
        <span
          class="absolute right-4 inset-y-0 flex items-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"
        >
          <svg
            class="w-4 h-4"
            fill="none"
    ...
    <div v-else class="text-center py-12">
      <div
        class="relative mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-green-500/10 ring-1 ring-green-500/50 mb-6"
      >
        <div
          class="absolute inset-0 rounded-full bg-green-500/20 animate-ping opacity-75"
        ></div>
        <svg
          class="h-10 w-10 text-green-400 relative z-10"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      <div class="mt-8 w-full bg-slate-800 rounded-full h-1 overflow-hidden">
        <div
          class="bg-indigo-500 h-1 rounded-full animate-[loading_1.5s_ease-in-out_infinite] w-1/3"
        ></div>
      </div>
    </div>
  </div>
